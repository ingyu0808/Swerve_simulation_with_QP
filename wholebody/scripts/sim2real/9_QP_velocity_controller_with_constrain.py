...
def step_robot(q, Tep, r, robot_model, ax=None):
    import math  # math 추가
    ...
    # 시각화 (matplotlib)
    if USE_VISUALIZATION and ax is not None:
        ax.cla()
        ee_pos = wTe[:3, -1]
        target_pos = Tep[:3, -1]
    ...
    # QP 구성
    n = 7  # Franka arm 7 DOF
    Q = np.eye(n + 6)
    Q[:n, :n] *= 0.01
    Q[n:, n:] = (1.0 / et) * np.eye(6)

    # ✅ Equality constraint: EE velocity tracking
    Aeq = np.c_[r.jacobe(q), np.eye(6)]
    beq = v.reshape((6,))

    # ✅ Inequality constraint: Joint limit avoidance
    Ain = np.zeros((n + 6, n + 6))
    bin = np.zeros(n + 6)

    ps = 0.1  # safe zone (radian)
    pi = 0.9  # influence zone (radian)
    Ain[:n, :n], bin[:n] = r.joint_velocity_damper(ps, pi, n)

    # ✅ Linear term: manipulability only (팔 관절만 최적화)
    c = np.concatenate(
        (-r.jacobm().reshape((n,)), np.zeros(6))  # 7개 관절만 manipulability optimization
    )
