...
def step_robot(q, Tep, frankie, robot_model, ax=None):
    ...
    print("RTB EE R:\n", frankie.fkine(q).A[:3, :3])
    print("Polymetis EE R:\n", wTe[:3, :3])
    ...
    jacobe1 = robot.robot_model.compute_jacobian(torch.Tensor(q), "panda_link8").numpy()
    jacobe2 = frankie.jacobe(q, end="panda_link8")

    Ad = sm.SE3.RPY([np.pi, np.pi/2, np.pi/2]).Ad()
    jacobe1_corrected = Ad @ jacobe1

    # jacobe1_spatial = Ad_T @ jacobe1

    print("Tensor q: ", torch.Tensor(q))
    print("q: ", q)
